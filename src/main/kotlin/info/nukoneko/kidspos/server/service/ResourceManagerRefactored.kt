...
class ResourceManagerRefactored(
    ...
    fun readFileContent(filePath: String, onClose: (() -> Unit)? = null): String =
        fileManager.readFileContent(filePath, onClose)
    ...
    fun processLinesFromStream(inputStream: java.io.InputStream, processor: (String) -> Unit) =
    ...
    fun cleanupOldBackups(directory: String, maxAge: Long, maxCount: Int): Int =
        backupManager.cleanupOldBackups(directory, maxAge, maxCount)

    // High-level operations combining multiple services
    fun safeFileOperation(sourcePath: String, destinationPath: String): Boolean {
        return try {
