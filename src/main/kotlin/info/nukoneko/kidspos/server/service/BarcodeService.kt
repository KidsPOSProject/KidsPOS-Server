...
class BarcodeService {
     ...
     */
    fun generateBarcodePdf(items: List<ItemEntity>, showBorders: Boolean = false): ByteArray {
    ...
    private fun createBarcodeCell(item: ItemEntity, showBorders: Boolean = false, font: PdfFont): com.itextpdf.layout.element.Cell {
        ...
        val cell = com.itextpdf.layout.element.Cell()
            ...
            .setBorder(if (showBorders) com.itextpdf.layout.borders.SolidBorder(0.5f) else com.itextpdf.layout.borders.Border.NO_BORDER)

        // セル内のレイアウト用テーブル（単純化）
        val innerTable = Table(1)
        ...
        if (barcodeImage != null) {
            ...
            val image = Image(ImageDataFactory.create(barcodeImage))
